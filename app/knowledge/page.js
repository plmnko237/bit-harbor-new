...
export default function knowledge() {
  ...
  useEffect(() => {
    ...
    if (currentCategory === "전체") {
      apiUrl = `https://server.bit-harbor.net/knowledge?page=${page}&size=${size}`;
    } else {
      apiUrl = `https://server.bit-harbor.net/knowledge/category?page=${page}&size=${size}&category=${currentCategory}`;
    }
  ...
  // 검색 처리
  const handleSearch = () => {
    const apiUrl = `https://server.bit-harbor.net/knowledge/search?page=0&size=10&keyword=${word}`;
    fetchData(apiUrl);
  };
          ...
          <div className="topSec">
            ...
            <div className="topSec_detail">
              <div className="secMenu">
                {["전체", "Tech뉴스", "팁", "칼럼", "리뷰"].map(
                  (category, i) => (
                    <span
                      key={i}
                      className={
                        currentCategory === category ? "tabActive" : ""
                      }
