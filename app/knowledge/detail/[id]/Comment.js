  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(
        `https://server.bit-harbor.net/knowledge/${dataItem.knowledgeId}/comments`,
        {
          method: "GET",
          headers: {
        ...
        comment.map((a, i) => (
          <div className="commit" key={i}>
            <div className="crumbs">
              ...
              {session && session.user.userNickname === comment[i].nickName ? (
                <div className="edit_del">
                  ...
                  <div
                    onClick={async (e) => {
                      let commentConfirm = confirm("댓글을 삭제하시겠습니까?");
                      if (commentConfirm) {
                        try {
                          await fetch(
                            `https://server.bit-harbor.net/knowledge/${dataItem.knowledgeId}/comment/${comment[i].commentId}`,
                            {
                              method: "DELETE",
                              headers: {
                                "Content-Type": "application/json",
            ...
            if (session && session.user) {
              if (content.trim() === "") {
                ...
                try {
                  // 새로운 댓글 서버에 등록
                  await fetch(
                    `https://server.bit-harbor.net/knowledge/${dataItem.knowledgeId}/comment`,
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
